...
experience = (root / 'assets/experience.bundle.js').read_text()
def data_url(path):
    file = root / path
...
html = html.replace('<script src="assets/experience.bundle.js" defer></script>', '')
...
html = html.replace('</body>', '<script>' + js + '</script><script>' + experience + '</script></body>')
...
with zipfile.ZipFile(root / 'portfolio.zip', 'w', zipfile.ZIP_DEFLATED) as archive:
    for name in ['index.html', 'style.css', 'script.js', 'experience.js', 'package.json', 'package-lock.json', 'README.md', 'SOURCES.md', 'build.py', 'portfolio-standalone.html']:
